...
policy=json.loads(POLICY.read_text()); version=int(policy['validation_version']); fallback=policy['rules']['unverified_fallback']
...
# Product detail: every SKU explicitly becomes either a verified image or a review placeholder.
for page in (DIST/'produk').glob('*/index.html'):
    slug=page.parent.name; t=page.read_text()
    v=mp.get(slug,{})
    brand=v.get('brand','')
    if slug in resolved:
        inner=f'<span>{esc(brand)}</span>{img_tag(slug,v.get("name","Foto produk"))}'
    else:
        inner=f'<span>{esc(brand)}</span><div class="product-image-placeholder detail"><span>Foto produk</span><strong>sedang diverifikasi</strong><small>Kami menahan gambar yang belum cocok dengan model produk.</small></div>'
...
# Product cards: never leave an old/unverified web image behind.
...
for page in DIST.rglob('*.html'):
    ...
    def card_repl(m):
        ...
        if slug in resolved:
            body=f'<span class="visual-brand">{esc(v.get("brand",""))}</span>'+img_tag(slug,v.get('name','Foto produk'))
        else:
            body=placeholder(v.get('brand',''))
