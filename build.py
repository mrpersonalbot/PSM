...
if dist.exists() and human_css.exists():
    ...
    # This lockup is cropped from the user's supplied 1536×1024 brand guide at
    # x=48, y=349, width=246, height=70, then locally upscaled sixfold before
    # export. Keep it local so the public site never relies on a remote image URL.
    supplied_wordmark = root / "source" / "assets" / "pratama-brand-wordmark-v3.png"
    if not supplied_wordmark.exists():
        raise RuntimeError("Missing supplied Pratama brand-wordmark asset")
    (assets / "pratama-brand-wordmark.jpg").unlink(missing_ok=True)
    shutil.copy2(supplied_wordmark, assets / "pratama-brand-wordmark-v3.png")
    ...
    for html_path in dist.rglob("*.html"): 
        ...
        # updated supplied wordmark treatment immediately.
        text = text.replace('/assets/human-touch.css', '/assets/human-touch.css?v=pratama-brand-guide-3')
        text = text.replace('/assets/pratama-logo.svg', '/assets/pratama-brand-wordmark-v3.png')
